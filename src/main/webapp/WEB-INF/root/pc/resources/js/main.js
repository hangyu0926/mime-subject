    ...
    function init() {
        return {
            accountQueryInit: function () {// 账号查看初始化
                ...
                $('#accountQueryAuthorityLevel').find('i').unbind('click').click(function (event) {// 点击权限级别下拉列表
                ...
                $('#accountQueryDatagridOrderByDate').find('i').unbind('click').click(function () {// 点击创建时间排序
                    var timeOrderType = parseInt($(this).attr('data-order'));
                    if (timeOrderType == 0) {
                        timeOrderType = 1;
                    } else {
                        timeOrderType = 0;
                    }
                    $(this).attr('data-order', timeOrderType);
                    ...
                    if (isSubmit) {
                        ...
                        global_ajax('batchImportAccount', params, function (data) {
                            ...
                            for (var i = 0; i < data.detailInfo.addFailList.length; i++) {
                                ...
                                tr.find('.xy-index-body-content-accountInput-select-show').text(permissionObj[failObj.permissionId]).attr(failObj.permissionId);
            ...
            projectQueryInit: function () {// 项目查看初始化
                ...
                $('.xy-index-body-content-projectQuery-filter').unbind('click').click(function () {// 点击筛选
                    if ($(this).hasClass('xy-index-body-content-projectQuery-filter-checked')) {
                        ...
                        $(".xy-index-body-content-choose-contant-small-check").unbind('click').click(function () {
                            if (!$(this).hasClass("isChoose")) {
                                $(this).next().next().addClass("chooseColor");
                                $(this).next().next().removeClass("notChoosecolor");
                                $(this).addClass("isChoose");
                            } else {
                                $(this).next().next().addClass("notChoosecolor");
                        ...
                        $(".xy-index-body-content-choose-contant-big-check").unbind('click').click(function () {
                            if (!$(this).hasClass("allChoose")) {
                                ...
                                $(this).parent().parent().find("span").addClass("chooseColor");
                                $(this).parent().parent().find("span").removeClass("notChoosecolor")
                                $(this).parent().parent().find("input").addClass("isChoose");
                            } else {
                                $(this).parent().parent().find("input").prop("checked", false);
                                $(this).removeClass("allChoose");
                                $(this).parent().parent().find("span").addClass("notChoosecolor");
                                $(this).parent().parent().find("input").removeClass("isChoose");
                            }
                        });
                    }
                })
...
function projectInputEventInit(_type, _projectId) {
    ...
    $('#projectInput_submit').unbind('click').click(function () {// 点击提交
        ...
        if (!global_validate.stringLessThanLength(summary, 20)) {
            global_dialog.error('项目概述必须为 1-20个字 ！');
            return;
        }
